...
{
    /**
     * Retourne la liste des exercices pour une entreprise donnée
     */
    public function getExercices(Request $request)
    {
        try {
            ...
            if (!$companyId) {
                return response()->json(['status' => 'error', 'message' => 'company_id requis'], 400);
            }

            $exercices = DB::table('exercices_comptables')
                ->where('company_id', $companyId)
                ->select('id', 'intitule as name', 'date_debut', 'date_fin')
    ...
    public function kpis(Request $request)
    {
        try {
            ...
            // Exercice comptable sélectionné (ou actif par défaut)
            $exerciceId = $request->input('exercice_id');
            $queryExercice = DB::table('exercices_comptables')
                ->where('company_id', $companyId);

            if ($exerciceId) {
                $exercice = $queryExercice->where('id', $exerciceId)->first();
            } else {
                $exercice = $queryExercice->where('cloturer', 0)->orderBy('date_debut', 'desc')->first();
            }
            ...
            // =========================================================
            // 2. REVENUS (Classe 7 SYSCOHADA) - Tous les revenus
            //    Les produits s'accumulent en CRÉDIT, non cumulatif (juste l'exercice)
            // =========================================================
            $revenus = -$this->calculerSoldeCompte($companyId, '7', $dateDebut, $dateFin, false);

            // =========================================================
            // 3. DÉPENSES / CHARGES (Classe 6 SYSCOHADA) - Toutes les charges
            //    Les charges s'accumulent en DÉBIT, non cumulatif (juste l'exercice)
            // =========================================================
            $depenses = $this->calculerSoldeCompte($companyId, '6', $dateDebut, $dateFin, false);
     ...
     */
    private function calculerSoldeCompte(int $companyId, string $prefix, string $dateDebut, string $dateFin, bool $isCumulative = true): float
    {
        ...
        if ($isCumulative) {
            $query->where('e.date', '<=', $dateFin);
        } else {
            $query->whereBetween('e.date', [$dateDebut, $dateFin]);
        }
