...
class PlanTiersEcritureController extends Controller
{
    //
    public function index(Request $request)
    {
        ...
        // Ajouter le filtre sur plan_tiers_id si fourni
        if (!empty($data['id_plan_tiers'])) {
            $query->where('plan_tiers_id', $data['id_plan_tiers']);
        }

        // Cloner la requête pour les sommes
        ...
        return view('plan_tiers_ecritures', compact(
            'data',
            'ecritures',
            'totalDebit',
            'totalCredit',
