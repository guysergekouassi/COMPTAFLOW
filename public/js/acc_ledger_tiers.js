document.addEventListener("DOMContentLoaded", function () {
    ...
    const resetFormFields = () => {
        inputs.forEach((input) => {
            input.value = "";
            input.classList.remove("is-invalid");
            $('#plan_tiers_id_1').selectpicker('val', '');
            $('#plan_tiers_id_2').selectpicker('val', '');
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", function (event) {
        ...
        if (compte1Num && compte2Num && compte1Num > compte2Num) {
            ...
            errorDiv.innerText =
                "Le tiers 2 doit être supérieur ou égal au tiers 1.";
...
document.getElementById("btnPreview").addEventListener("click", function (event) {
    ...
    fetch(accounting_ledgerpreviewGrandLivreTiersUrl, {   // Mets bien la bonne URL
        method: "POST",
        body: formData,
        headers: {
