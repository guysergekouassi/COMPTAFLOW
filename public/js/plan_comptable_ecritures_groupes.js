function exporterTableau2() {
    ...
    // alert(plan_comptable_ecritures_groupesUpdateBaseUrl);
    document.getElementById('modalLoaderOverlay').classList.remove('d-none');

    // --- On envoie à Laravel si équilibré ---
    fetch(plan_comptable_ecritures_groupesUpdateBaseUrl, {
        ...
        .then(async (response) => {
            ...
            document.getElementById("groupSuccessMessage").textContent = result.message;
            document.getElementById("groupSuccessAlert").classList.remove("d-none");

            setTimeout(() => {
                document.getElementById("groupSuccessAlert").classList.add("d-none");
                location.reload();
            }, 3000);
        })
